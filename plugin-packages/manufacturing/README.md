...
does is a sequence of operations. Every cycle shown beside a layout there is the
one measured **at that machine** rather than one averaged across every machine the
layout runs on, and pressing a pocket under it puts the watch on that tool, at
that machine — the same sentence the floor's machine buttons say, arrived at from
the other end. Ask with a part number, a tool number or **TL 12** instead and the
